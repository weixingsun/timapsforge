...
import static ti.mapsforge.MathUtil.*;
...
public class PolyUtil {
     ...
     */
    public static boolean containsLocation(LatLong point, List<LatLong> polygon, boolean geodesic) {
        ...
        LatLong prev = polygon.get(size - 1);
        double lat1 = toRadians(prev.latitude);
        double lng1 = toRadians(prev.longitude);
        int nIntersect = 0;
        for (LatLong point2 : polygon) {
     ...
     */
    public static boolean isLocationOnEdge(LatLong point, List<LatLong> polygon, boolean geodesic,
                                           double tolerance) {
     ...
     */
    public static boolean isLocationOnPath(LatLong point, List<LatLong> polyline,
    ...
    private static boolean isLocationOnEdgeOrPath(LatLong point, List<LatLong> poly, boolean closed,
        ...
        double lat3 = toRadians(point.latitude);
        double lng3 = toRadians(point.longitude);
        LatLong prev = poly.get(closed ? size - 1 : 0);
        double lat1 = toRadians(prev.latitude);
        double lng1 = toRadians(prev.longitude);
        if (geodesic) {
            for (LatLong point2 : poly) {
                double lat2 = toRadians(point2.latitude);
                double lng2 = toRadians(point2.longitude);
     ...
     */
    public static List<LatLong> decode(final String encodedPath) {
        int len = encodedPath.length();

        // For speed we preallocate to an upper bound on the final length, then
        // truncate the array before returning.
        final List<LatLong> path = new ArrayList<LatLong>();
        int index = 0;
        int lat = 0;
        int lng = 0;
        ...
        while (index < len) {
            ...
            path.add(new LatLong(lat * 1e-5, lng * 1e-5));
        }
     ...
     */
    public static String encode(final List<LatLong> path) {
        long lastLat = 0;
        long lastLng = 0;

        final StringBuffer result = new StringBuffer();

        for (final LatLong point : path) {
